 ...
 * 可重试的错误码（网络层）
 */
const RETRYABLE_CODES = ['ETIMEDOUT', 'ESOCKETTIMEDOUT', 'ECONNREFUSED', 'ENOTFOUND', 'ECONNRESET', 'EAI_AGAIN']
...
function mapError(error, context = {}) {
  ...
  // 4. HTTP 429: 限流（可重试）
  if (status === 429) {
    const finalRetryAfter = retryAfter || 2
    console.log('[error-mapping] 429 status, retryAfter:', retryAfter, 'finalRetryAfter:', finalRetryAfter)
    return new LLMAPIError(
      `请求频率受限: ${errorMessage}`,
      429,
      true,
