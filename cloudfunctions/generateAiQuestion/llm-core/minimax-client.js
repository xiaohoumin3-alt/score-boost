...
class MiniMaxClient {
  ...
  async _call(requestBody, signal) {
    ...
    console.log('[MiniMaxClient] Calling API URL:', url)
    console.log('[MiniMaxClient] Request body:', JSON.stringify(requestBody).slice(0, 300))
    ...
    try {
      ...
      console.log('[MiniMaxClient] Response status:', response.status, 'ok:', response.ok)
    } catch (fetchError) {
      console.error('[MiniMaxClient] Fetch error:', fetchError.message)
      throw fetchError
    }
