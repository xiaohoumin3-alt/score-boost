 ...
 *
 * 默认配置：
 * - baseDelay: 2000ms（基础延迟，大幅增加）
 * - maxDelay: 120000ms（最大延迟，可等待2分钟）
 * - maxRetries: 10（最大重试次数，大幅增加应对持续限流）
 */
...
async function retryWithBackoff(fn, options = {}) {
  const {
    maxRetries = 10,  // 大幅增加重试次数以应对持续限流
    baseDelay = 2000,  // 基础延迟2秒
    maxDelay = 120000, // 最大延迟2分钟
    onRetry = null
  } = options
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log('[retry] Attempt', attempt, 'error.retryAfter:', error.retryAfter, 'delay:', delay, 'actualDelay:', actualDelay)
