...
function loadKnowledgeTree(subject, grade, semester = '下') {
  ...
  const gradeKey = gradeMap[grade] || String(grade || '8');
  const semesterKey = semesterMap[semester] || semester || 'down';

  try {
    const functionDir = path.dirname(__dirname);
    const dataDirs = [
      path.join(functionDir, 'data'),
      path.join(functionDir, '..', 'startAssessment', 'data')
    ];

    // 尝试加载科目对应的数据文件
    for (const dataDir of dataDirs) {
      const dataFile = path.join(dataDir, `${dbSubject}-grade${gradeKey}-${semesterKey}.json`);
    ...
    // 回退到内嵌数据，根据科目返回不同知识树
    const embedded = getEmbeddedData(dbSubject, gradeKey);
    if (embedded && embedded.chapters && embedded.chapters.length > 0) {
      return embedded;
    }

    // 内嵌数据也没有，返回空树（调用方会处理）
    return { subject: dbSubject, grade: gradeKey, semester: semesterKey, chapters: [] };
  } catch (e) {
    return getEmbeddedData(dbSubject, gradeKey) || { subject: dbSubject, grade: gradeKey, semester: semesterKey, chapters: [] };
  }
}
