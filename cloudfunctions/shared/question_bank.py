...
QUESTION_BANK: Dict[str, List[Dict]] = {
    ...
    "kp4_1": [  # 函数概念
        ...
        {"content": "下列哪个是函数关系？", "options": ["A. y=±√x", "B. y=2x", "C. x=y²", "D. x²+y²=1"], "correct_answer": "B", "difficulty": "medium"},
...
def generate_questions(plan: List[Dict], num_questions: int = 5) -> List[Dict]:
    """根据出题计划从题库生成题目"""
    questions = []
    kp_count = {}

    for item in plan[:num_questions]:
        kp_id = item["kp"]["kp_id"]
        difficulty = item["difficulty"]

        # 记录该知识点的使用次数
        if kp_id not in kp_count:
            kp_count[kp_id] = 0

        # 从题库获取题目
        bank = QUESTION_BANK.get(kp_id, [])
        if bank:
            # 优先选择匹配难度的题目
            matching = [q for q in bank if q["difficulty"] == difficulty]
            if matching:
            ...
            questions.append({
                "id": f"q{kp_count[kp_id] + 1}_{kp_id}",
                "type": "choice",
                "content": q["content"],
                "options": q["options"],
